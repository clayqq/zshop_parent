...
import com.itany.zshop.common.constant.SysuserContant;
...
public class SysuserServiceImpl implements SysuserService {
    ...
    public void add(SysuserVo sysuserVo) {
        ...
        try {
            ...
            //角色ID
            Role role = new Role();
            role.setId(sysuserVo.getId());
            sysuser.setRole(role);
            //默认为有效
            sysuser.setIsValid(SysuserContant.SYSUSER_VALID);
            //创建时间为当前时间
    ...
    public void modifyStatus(int id) {
        ...
        if(isValid == SysuserContant.SYSUSER_VALID){
            isValid = SysuserContant.SYSUSER_INVALID;
        }else{
            isValid = SysuserContant.SYSUSER_VALID;
        }
    ...
    public Sysuser login(String loginName, String password) throws SysuserNotExistException {
        Sysuser sysuser = sysuserDao.selectByLoginNameAndPassword(loginName,password,SysuserContant.SYSUSER_VALID);
        if(sysuser != null){
            return sysuser;
        }
